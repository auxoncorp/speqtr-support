...
const EVENT_COORDS_TERMINAL_LINK_PROVIDER: vscode.TerminalLinkProvider = {
    provideTerminalLinks: (context: vscode.TerminalLinkContext, token: vscode.CancellationToken) => {
        var links = [];
        const event_coords_re = RegExp("%[0-9a-f]{32}:[0-9a-f]+", "g");

        let exec_res_array = event_coords_re.exec(context.line);
        if (exec_res_array != null) {
            for (const exec_res of exec_res_array) {
                links.push({
                    startIndex: exec_res_array.index,
                    length: exec_res.length,
    ...
    handleTerminalLink: (link: any) => {
        vscode.commands.executeCommand(MODALITY_LOG_COMMAND, { type: "LogCommandArgs", around: link.data, radius: 5 });
    }
};
